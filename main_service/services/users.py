...
class UserService:
    ...
    def money_accounts(self, id_green, id_yellow, id_red):
        ...
        if id_green:
            green_account_df, _ = self.get_account_info('http://https://mrcool-greenbank.herokuapp.com', id_green)
        ...
        yellow_account_df, _ = pd.DataFrame(columns=('number', 'type', 'amount')), \
                                               pd.DataFrame(columns=('date', 'amount', 'income', 'payment_kind'))
        if id_yellow:
            yellow_account_df, _ = self.get_account_info('http://https://mrcool-yellowbank.herokuapp.com',
                                                                    id_yellow)
        ...
        red_account_df, _ = pd.DataFrame(columns=('number', 'type', 'amount')), \
                                         pd.DataFrame(columns=('date', 'amount', 'income', 'payment_kind'))
        if id_red:
            red_account_df, _ = self.get_account_info('http://https://mrcool-redbank.herokuapp.com', id_red)
    ...
    def money_history(self, id_green, id_yellow, id_red):
        ...
        _, green_account_df = pd.DataFrame(columns=('number', 'type', 'amount')), \
                                             pd.DataFrame(columns=('date', 'amount', 'income', 'payment_kind'))
        if id_green:
            _, green_account_df = self.get_account_info('http://https://mrcool-greenbank.herokuapp.com', id_green)
        green_account_df['bank'] = 'green'
        ...
        _, yellow_account_df = pd.DataFrame(columns=('number', 'type', 'amount')), \
                                               pd.DataFrame(columns=('date', 'amount', 'income', 'payment_kind'))
        if id_yellow:
            _, yellow_account_df = self.get_account_info('http://https://mrcool-yellowbank.herokuapp.com', id_yellow)
        yellow_account_df['bank'] = 'yellow'
        ...
        _, red_account_df = pd.DataFrame(columns=('number', 'type', 'amount')), \
                                         pd.DataFrame(columns=('date', 'amount', 'income', 'payment_kind'))
        if id_red:
            _, red_account_df = self.get_account_info('http://https://mrcool-redbank.herokuapp.com', id_red)
        red_account_df['bank'] = 'red'
    ...
    def money_report(self, id_green, id_yellow, id_red):
        ...
        green_account_df, green_history_df = pd.DataFrame(columns=('number', 'type', 'amount')), \
                                             pd.DataFrame(columns=('date', 'amount', 'income', 'payment_kind'))
        if id_green:
            green_account_df, green_history_df = self.get_account_info('http://https://mrcool-greenbank.herokuapp.com', id_green)

        # yellow
        yellow_account_df, yellow_history_df = pd.DataFrame(columns=('number', 'type', 'amount')), \
                                               pd.DataFrame(columns=('date', 'amount', 'income', 'payment_kind'))
        if id_yellow:
            yellow_account_df, yellow_history_df = self.get_account_info('http://https://mrcool-yellowbank.herokuapp.com',
                                                                    id_yellow)
        ...
        red_account_df, red_history_df = pd.DataFrame(columns=('number', 'type', 'amount')), \
                                         pd.DataFrame(columns=('date', 'amount', 'income', 'payment_kind'))
        if id_red:
            red_account_df, red_history_df = self.get_account_info('http://https://mrcool-redbank.herokuapp.com', id_red)
